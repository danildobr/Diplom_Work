...
class DeliveryAddressSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if DeliveryAddress.objects.filter(
            user=user, city=city, street=street, house=house, apartment=apartment
        ).exists():
            raise ValidationError(
                "Адрес доставки с такими параметрами уже существует для этого пользователя."
            )
        return attrs
...
class UserSerializer(serializers.ModelSerializer):
    ...
    password = serializers.CharField(
        write_only=True
    )  # Пароль не будет возвращаться в JSON
